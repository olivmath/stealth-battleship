// Translator — turns proof verify HTTP boundary
...
export function createTurnsProofVerifyRouter(port?: TurnsProofVerifyPort): Router {
  const router = Router();
  const adapter = port ?? createTurnsProofVerifyAdapter();
  ...
  router.post('/turns-proof', async (req: Request, res: Response) => {
    ...
    try {
      ...
      const validation = validateTurnsProofVerifyInput(req.body);
...
export default createTurnsProofVerifyRouter();
