...
export default function PvPPlacementScreen() {
  ...
  // When both ready, navigate to battle
  useEffect(() => {
  ...
  }, [selectedShip, haptics, playerReady]);
  ...
  const handleGridTouchStart = useCallback((e: GestureResponderEvent) => {
    ...
    updatePreview(pos);
  }, [selectedShip, state.playerBoard, positionFromTouch, updatePreview, haptics, dispatch, playerReady]);

  const handleGridTouchMove = useCallback((e: GestureResponderEvent) => {
    if (!selectedShip || playerReady) return;
    const { pageX, pageY } = e.nativeEvent;
    const pos = positionFromTouch(pageX, pageY);
    updatePreview(pos);
  }, [selectedShip, positionFromTouch, updatePreview, playerReady]);

  const handleGridTouchEnd = useCallback(() => {
    if (!selectedShip || previewPositions.length === 0 || playerReady) return;
    ...
    setPreviewPositions([]);
    setPreviewValid(false);
  }, [selectedShip, previewPositions, previewValid, orientation, dispatch, haptics, playerReady]);
  ...
  const handleReady = () => {
    ...
    // Mock opponent becoming ready after delay
  ...
  const opponentStatus = playerReady
    ? (opponentReady ? 'ready' : 'online')
    : 'online';
  ...
  return (
    <GradientContainer>
      <View style={styles.container}>
        {/* Opponent status */}
        <OpponentStatus name={MOCK_OPPONENT} status={opponentStatus} />
          ...
          <Text style={styles.subtitle}>
            {playerReady
              ...
              : selectedShip
                ...
                : allPlaced
                  ? 'All ships deployed! Tap READY when done.'
